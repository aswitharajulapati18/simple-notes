...
def viewallfiles():
    try:
        ...
        cursor.execute('select userid from users where useremail=%s',[session.get('user')])
        uid=cursor.fetchone()
        cursor.execute('select f_id,filename,created_at from filedata where added_by=%s',[uid[0]])
...
def viewfile(fid):
    try:
        cursor=mydb.cursor(buffered=True)
        cursor.execute('select filename,fdata from filedata where f_id=%s',[fid])
...
def deletefile(fid):
    try:
        cursor=mydb.cursor(buffered=True)
        cursor.execute('delete from filedata where f_id=%s',[fid])
        mydb.commit()
        cursor.close()
    except Exception as e:
        print(e)
